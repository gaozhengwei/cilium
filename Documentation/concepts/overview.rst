...
Agent
  The Hubble agent/server runs on each node and retrieves the eBPF-based
...
Relay
  ...
  their respective gRPC APIs and providing an API that represents all agents in
  the cluster.

Client (CLI)
  The Hubble CLI (``hubble``) connect to the gRPC API of relay or the agent and
  retrieves flow events.
...
in the kernel such as for incoming packets, outgoing packets, system calls,
kprobes, etc.
